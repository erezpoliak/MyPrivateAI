...
class PDFParser:
    ...
    Usage::
        ...
        result = parser.parse(Path("data/papers/10.1038__s41467-024-44750-0.pdf"))
        if result.success:
    ...
    def __init__(self, config: Config | None = None) -> None:
        ...
        self._papers_dir = config.papers_dir
    ...
    def parse_all(self) -> list[ParseResult]:
        """Parse every PDF in ``config.papers_dir``, logging progress."""
        paths = sorted(self._papers_dir.glob("*.pdf"))
        if not paths:
            logger.warning("No PDFs found in %s", self._papers_dir)
            return []
